...
std::uint32_t read_png_u32(const std::string &png, std::size_t offset) {
  ...
  const auto *data =
      reinterpret_cast<const unsigned char *>(png.data() + offset);
...
std::uint32_t png_crc(const std::string &png, std::size_t offset,
                      std::size_t size) {
...
std::size_t find_png_chunk(const std::string &png, const char *type) {
  ...
  while (offset <= png.size() && png.size() - offset >= 12) {
    ...
    if (png.compare(offset + 4, 4, type) == 0)
      return offset;
    offset += length + 12;
  }
  return std::string::npos;
...
bool has_argument_pair(const std::vector<std::string> &arguments,
...
Fixture make_registry_fixture(
    const std::string &source_repo = "https://github.com/LILYGO-UI/demo") {
  const std::string detail_url = std::string(kBase) + "/v1/snapshots/" +
                                 kSnapshot + "/apps/cc.lilygo.ui.Demo.json";
  ...
  const std::string index_url =
      std::string(kBase) + "/v1/snapshots/" + kSnapshot + "/index.json";
...
public:
  ...
  HttpTextResult get(const std::string &url, std::size_t maximum_size,
                     bool) override {
    const auto found = documents.find(url);
    if (found == documents.end())
      return {{}, "not found"};
    if (found->second.size() > maximum_size)
      return {{}, "too large"};
    return {found->second, {}};
  }

  HttpDownloadResult download(const std::string &url,
                              const std::filesystem::path &destination,
                              std::uint64_t maximum_size, bool) override {
    download_urls.push_back(url);
    downloaded = true;
    const auto failed = download_errors.find(url);
    if (failed != download_errors.end())
      return {failed->second};
    if (package_content.size() > maximum_size)
      return {"too large"};
...
public:
  ...
  ProcessResult run(const std::vector<std::string> &arguments,
    ...
    if (arguments.size() >= 2 && arguments[0] == "dpkg" &&
        arguments[1] == "--print-architecture")
      return {0, "arm64\n", {}, false};
    if (!arguments.empty() && arguments[0] == "dpkg-query") {
      const auto found = installed.find(arguments.back());
      if (found == installed.end())
        return {1, {}, "not installed", false};
      return {0, "ii \t" + found->second, {}, false};
    }
    if (!arguments.empty() && arguments[0] == "pkexec") {
      if (arguments.size() > 2 && arguments[2] == "--install") {
...
StoreApp make_store_app(std::string id, bool installed = false,
                        bool update = false) {
  StoreApp app;
  app.id = std::move(id);
...
void test_registry_snapshot_protocol() {
  ...
  const auto loaded_from_cached_root =
      client.load_document(fixture.root_url, cached_root);
  ...
  fixture = make_registry_fixture();
  const auto lower_index_url =
      std::string(kBase) + "/v1/snapshots/" + kSnapshot + "/index.json";
  const auto upper_index_url =
      std::string("https://REGISTRY.TEST/v1/snapshots/") + kSnapshot +
      "/index.json";
  const auto lower_detail_url = std::string(kBase) + "/v1/snapshots/" +
                                kSnapshot + "/apps/cc.lilygo.ui.Demo.json";
  const auto upper_detail_url =
      std::string("https://REGISTRY.TEST/v1/snapshots/") + kSnapshot +
      "/apps/cc.lilygo.ui.Demo.json";
  auto index_with_uppercase_host = fixture.documents.at(lower_index_url);
  const auto detail_url_position =
      index_with_uppercase_host.find(lower_detail_url);
  assert(detail_url_position != std::string::npos);
  index_with_uppercase_host.replace(detail_url_position,
                                    lower_detail_url.size(), upper_detail_url);
  auto root_with_uppercase_host = fixture.documents.at(fixture.root_url);
  const auto index_url_position =
      root_with_uppercase_host.find(lower_index_url);
  assert(index_url_position != std::string::npos);
  root_with_uppercase_host.replace(index_url_position, lower_index_url.size(),
                                   upper_index_url);
  const auto uppercase_hash_position =
      root_with_uppercase_host.find("\"sha256\":\"");
  assert(uppercase_hash_position != std::string::npos);
  root_with_uppercase_host.replace(uppercase_hash_position + 10, 64,
                                   sha256(index_with_uppercase_host));
  ...
  changed_index.replace(timestamp_position, old_timestamp.size(),
                        new_timestamp);
  ...
  changed_root.replace(hash_position + hash_marker.size(), 64,
                       sha256(changed_index));
  ...
  fixture = make_registry_fixture();
  http.documents = fixture.documents;
  const auto detail_url = std::string(kBase) + "/v1/snapshots/" + kSnapshot +
                          "/apps/cc.lilygo.ui.Demo.json";
  ...
  changed_detail.replace(title_position,
                         std::string("\"title\":\"演示\"").size(),
                         "\"title\":\"冒名应用\"");
...
void test_store_service_and_package_verification() {
  ...
  assert(refreshed.apps.front().icon_cache_key.find(kSnapshot) !=
         std::string::npos);
  assert(refreshed.apps.front().icon_cache_key.find(
             refreshed.apps.front().icon_url) != std::string::npos);
  assert(refreshed.apps.front().screenshots.size() == 1);
  assert(refreshed.apps.front().screenshots.front().url ==
         "https://registry.test/assets/screen.png");
  assert(refreshed.apps.front().screenshots.front().caption == "主界面");
  assert(refreshed.apps.front().screenshots.front().cache_key.find(kSnapshot) !=
         std::string::npos);
  assert(refreshed.apps.front().screenshots.front().cache_key.find(
             refreshed.apps.front().screenshots.front().url) !=
         std::string::npos);
  assert(refreshed.apps.front().icon_cache_key !=
         refreshed.apps.front().screenshots.front().cache_key);

  auto community_fixture =
      make_registry_fixture("https://github.com/LILYGO-UI-Community/demo");
  ...
  const auto community_refreshed =
      community_service.refresh(community_fixture.root_url);
  assert(community_refreshed);
  assert(community_refreshed.apps.size() == 1);
  assert(!community_refreshed.apps.front().official);

  auto traversal_fixture =
      make_registry_fixture("https://github.com/LILYGO-UI/../attacker");
  community_http.documents = traversal_fixture.documents;
  const auto traversal_refreshed =
      community_service.refresh(traversal_fixture.root_url);
  assert(traversal_refreshed);
  assert(traversal_refreshed.apps.size() == 1);
  assert(!traversal_refreshed.apps.front().official);

  const auto installed =
      packages.install("lilygo-ui-demo", refreshed.apps.front().release);
  assert(installed);
...
void test_github_api_package_download() {
  ...
  const std::string release_url =
      "https://github.com/LILYGO-UI/packages/releases/download/" + tag + "/" +
      asset_name;
  const std::string metadata_url =
      "https://api.github.com/repos/LILYGO-UI/packages/releases/tags/" + tag;
  const std::string asset_url =
      "https://api.github.com/repos/LILYGO-UI/packages/releases/assets/12345";

  RegistryRelease release{
      "1.2.3", "arm64",     "published", 7,
      digest,  release_url, "0.1.0",     "2026-08-31T12:00:00+08:00",
      {}};
  FakeHttpClient http;
  http.documents[metadata_url] =
      "{\"tag_name\":\"" + tag + "\",\"assets\":[{\"name\":\"" + asset_name +
  ...
  fallback_http.documents[metadata_url] =
      "{\"tag_name\":\"" + tag + "\",\"assets\":[{\"name\":\"" + asset_name +
  ...
  origin_http.documents[metadata_url] =
      "{\"tag_name\":\"" + tag + "\",\"assets\":[{\"name\":\"" + asset_name +
      "\",\"size\":7,\"digest\":\"sha256:" + digest +
      "\",\"state\":\"uploaded\",\"url\":"
...
void test_registry_root_cache() {
  ...
  const auto test_root =
      ...
      ("lilygo-ui-store-root-cache-test-" + std::to_string(getpid()) + "-" +
       std::to_string(nonce));
  ...
  const std::string previous_cache =
      had_previous_cache ? previous_cache_value : std::string{};
...
void test_image_download_validation_and_curl_options() {
  ...
  const auto test_root =
      std::filesystem::temp_directory_path() /
      ("lilygo-ui-store-image-test-" + std::to_string(getpid()) + "-" +
       std::to_string(nonce));
  ...
  const auto first = download_store_image(source_url, std::string(kSnapshot) +
                                                          ":" + source_url);
  assert(first);
  assert(std::filesystem::is_regular_file(first.path));

  const auto second = download_store_image(
      source_url, std::string("different-snapshot:") + source_url);
  assert(second);
  ...
  const auto normalized = download_store_image(
      preview_url, std::string("normalized-preview:") + preview_url);
  assert(normalized);
  ...
  const auto normalized_width =
      read_png_u32(normalized_png, normalized_header + 8);
  const auto normalized_height =
      read_png_u32(normalized_png, normalized_header + 12);
  assert(static_cast<std::uint64_t>(normalized_width) * normalized_height <=
         240000U);
  ...
  const auto rejected_iend = download_store_image(
      missing_iend_url, std::string("missing-iend:") + missing_iend_url);
  ...
  bad_crc[image_data_marker + 4] =
      static_cast<char>(bad_crc[image_data_marker + 4] ^ 0x01);
  ...
  const auto rejected_crc =
      download_store_image(bad_crc_url, std::string("bad-crc:") + bad_crc_url);
  assert(!rejected_crc);
  ...
  const auto image_data_size = static_cast<std::size_t>(
      read_png_u32(invalid_image_data, image_data_chunk));
  assert(image_data_size > 2);
  const auto image_data_offset = image_data_chunk + 8;
  assert(static_cast<unsigned char>(invalid_image_data[image_data_offset]) !=
         0);
  invalid_image_data[image_data_offset] = 0;
  const auto recomputed_crc =
  ...
  assert(
      read_png_u32(invalid_image_data, crc_offset) ==
      png_crc(invalid_image_data, image_data_chunk + 4, image_data_size + 4));
  const auto invalid_image_data_path =
      test_root / "inputs" / "invalid-image-data.png";
  ...
  const auto rejected_decode = download_store_image(
      invalid_image_data_url,
      std::string("invalid-image-data:") + invalid_image_data_url);
  assert(!rejected_decode);
  assert(rejected_decode.path.empty());
  assert(rejected_decode.error.find("Unable to prepare image for display") !=
         std::string::npos);

  FakeProcessRunner curl_runner;
  CurlHttpClient curl(curl_runner);
  const auto binary =
      curl.get_binary("https://registry.test/assets/icon.png", 4096);
  assert(!binary);
  ...
  const auto downloaded =
      curl.download("https://registry.test/package.deb", destination, 4096);
  assert(!downloaded);
  assert(curl_runner.calls.size() == 2);
  assert(has_argument_pair(curl_runner.calls.back(), "--header",
