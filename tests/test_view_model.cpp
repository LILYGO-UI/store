...
StoreApp make_app(std::string id, bool installed = false,
...
int main() {
  lv_init();
  {
    ...
    official.screenshots.push_back(
        {"https://registry.example/official-preview.png",
         "Preview",
         "official-preview",
         {},
         {},
    ...
    assert_revision(revision, 6);
    assert(!store.complete_icon(official.app_id, "wrong-url",
                                official.icon_cache_key, "/tmp/icon.png", {}));
    assert_revision(revision, 6);
    assert(store.complete_icon(official.app_id, official.icon_url,
                               official.icon_cache_key, "/tmp/icon.png", {}));
    ...
    assert(!store.complete_screenshot(official.app_id, 0, "wrong-url",
                                      "official-preview", "/tmp/preview.png",
                                      {}));
    assert_revision(revision, 8);
    assert(store.complete_screenshot(
        official.app_id, 0, official.screenshots[0].url,
