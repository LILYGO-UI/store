...
public:
  PackageManager(ProcessRunner &runner, HttpClient &http) noexcept
      : runner_(runner), http_(http) {}

  [[nodiscard]] InstalledPackageResult query(const std::string &package);
  [[nodiscard]] std::string architecture();
  [[nodiscard]] PackageOperationResult install(const std::string &package,
