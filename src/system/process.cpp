...
void drain_fd(int &descriptor, std::string &destination, std::size_t limit,
              bool &limit_exceeded) {
  ...
  while (descriptor >= 0) {
    ...
    if (count > 0) {
      ...
      if (previous_size < limit) {
        ...
        destination.append(buffer.data(),
                           amount < remaining ? amount : remaining);
      }
      if (amount > limit - (previous_size < limit ? previous_size : limit))
        limit_exceeded = true;
      continue;
    }
    if (count == 0) {
...
ProcessResult PosixProcessRunner::run(const std::vector<std::string> &arguments,
  ...
  if (pipe(output_pipe) != 0 || pipe(error_pipe) != 0) {
    ...
    result.error =
        std::string("Unable to create process pipe: ") + std::strerror(errno);
    return result;
  }
  ...
  if (spawn_error != 0) {
    close_fd(output_pipe[0]);
    close_fd(error_pipe[0]);
    result.error = std::string("Unable to start ") + arguments.front() + ": " +
  ...
  while (output_pipe[0] >= 0 || error_pipe[0] >= 0) {
    pollfd descriptors[2]{{output_pipe[0], POLLIN | POLLHUP, 0},
                          {error_pipe[0], POLLIN | POLLHUP, 0}};
    const auto polled = poll(descriptors, 2, 1000);
    if (polled < 0 && errno != EINTR)
      break;
    drain_fd(output_pipe[0], result.output, output_limit,
             result.output_limit_exceeded);
    drain_fd(error_pipe[0], result.error, output_limit,
             result.output_limit_exceeded);
