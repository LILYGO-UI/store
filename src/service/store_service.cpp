...
public:
    explicit RecordingHttpClient(HttpClient &source) noexcept : source_(source)
    {
    }

    HttpTextResult get(const std::string &url, std::size_t maximum_size, bool allow_file) override
    {
        auto result = source_.get(url, maximum_size, allow_file);
        if (!result) return result;
        const auto found =
            std::find_if(documents_.begin(), documents_.end(), [&](const auto &item) { return item.url == url; });
        if (found == documents_.end())
            documents_.push_back({url, result.body});
        else
            found->body = result.body;
        return result;
    }

    HttpDownloadResult download(const std::string &url, const std::filesystem::path &destination,
                                std::uint64_t maximum_size, bool allow_file) override
    {
        return source_.download(url, destination, maximum_size, allow_file);
    }

    [[nodiscard]] const std::vector<RegistryCacheDocument> &documents() const
    {
        return documents_;
    }
...
private:
    HttpClient &source_;
    std::vector<RegistryCacheDocument> documents_;
};

class CachedRegistryHttpClient final : public HttpClient {
public:
    CachedRegistryHttpClient(const RegistryRootCache &cache, std::string root_url)
        : cache_(cache), root_url_(std::move(root_url))
    {
    }

    HttpTextResult get(const std::string &url, std::size_t maximum_size, bool allow_file) override
    {
        if (!valid_transport_url(url, allow_file)) return {{}, "Registry URL must use HTTPS"};
        auto cached = cache_.load_document(root_url_, url, maximum_size);
        if (!cached) return {{}, std::move(cached.error)};
        return {std::move(cached.document), {}};
    }

    HttpDownloadResult download(const std::string &, const std::filesystem::path &, std::uint64_t, bool) override
    {
        return {"Package downloads are not available from the Registry cache"};
    }

private:
    const RegistryRootCache &cache_;
...
std::string local_id(const std::string &package)
{
    ...
    return package.rfind(prefix, 0) == 0 ? package.substr(prefix.size()) : package;
}
...
bool official_source_repo(const std::string &source_repo)
{
    ...
    std::transform(source_repo.cbegin(), source_repo.cend(), std::back_inserter(normalized),
    ...
    if (!repository.empty() && repository.back() == '/') repository.pop_back();
    constexpr std::string_view git_suffix = ".git";
    if (repository.size() > git_suffix.size() &&
        repository.compare(repository.size() - git_suffix.size(), git_suffix.size(), git_suffix) == 0)
        repository.resize(repository.size() - git_suffix.size());
    return !repository.empty() && repository != "." && repository != ".." &&
           std::all_of(repository.cbegin(), repository.cend(),
...
bool valid_png_data(const unsigned char *data, std::size_t size, PngDimensions *dimensions = nullptr)
{
    static constexpr unsigned char signature[] = {0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a};
    if (size < 45 || !std::equal(std::begin(signature), std::end(signature), data)) return false;
    ...
    while (offset < size) {
        ...
        const auto length = static_cast<std::size_t>(read_big_endian(data + offset));
        ...
        if (png_crc(type, length + 4) != stored_crc) return false;
        ...
        if (!saw_header) {
            if (!is_header || length != 13) return false;
            ...
            if (width == 0 || width > 4096 || height == 0 || height > 4096 ||
                ...
                !valid_png_bit_depth(chunk_data[8], chunk_data[9]) || chunk_data[10] != 0 || chunk_data[11] != 0 ||
                chunk_data[12] > 1)
                return false;
            if (dimensions) *dimensions = {width, height};
            saw_header = true;
        } else if (is_header) {
            return false;
        }
        ...
        if (is_end) return length == 0 && saw_image_data && next == size;
        offset = next;
    }
    return false;
}
...
bool valid_cached_png(const std::filesystem::path &path)
{
    ...
    if (error || size < 45 || size > kMaximumImageBytes) return false;
    ...
    return valid_png_data(data.data(), data.size(), &dimensions) &&
           static_cast<std::uint64_t>(dimensions.width) * dimensions.height <= kCachedImageMaximumPixels;
}

std::vector<unsigned char> resize_rgba(const std::vector<unsigned char> &source, const PngDimensions &source_size,
                                       const PngDimensions &destination_size)
{
    std::vector<unsigned char> destination(static_cast<std::size_t>(destination_size.width) * destination_size.height *
                                           4U);
    for (std::uint32_t y = 0; y < destination_size.height; ++y) {
        const auto source_y =
            std::clamp((static_cast<double>(y) + 0.5) * source_size.height / destination_size.height - 0.5, 0.0,
                       static_cast<double>(source_size.height - 1));
        ...
        for (std::uint32_t x = 0; x < destination_size.width; ++x) {
            const auto source_x =
                std::clamp((static_cast<double>(x) + 0.5) * source_size.width / destination_size.width - 0.5, 0.0,
                           static_cast<double>(source_size.width - 1));
            ...
            for (std::size_t channel = 0; channel < 4; ++channel) {
                const auto top_left    = source[(static_cast<std::size_t>(y0) * source_size.width + x0) * 4U + channel];
                const auto top_right   = source[(static_cast<std::size_t>(y0) * source_size.width + x1) * 4U + channel];
                const auto bottom_left = source[(static_cast<std::size_t>(y1) * source_size.width + x0) * 4U + channel];
                const auto bottom_right =
                    source[(static_cast<std::size_t>(y1) * source_size.width + x1) * 4U + channel];
                const auto top    = top_left + (top_right - top_left) * x_weight;
                const auto bottom = bottom_left + (bottom_right - bottom_left) * x_weight;
                destination[(static_cast<std::size_t>(y) * destination_size.width + x) * 4U + channel] =
                    static_cast<unsigned char>(std::lround(top + (bottom - top) * y_weight));
            }
        }
    }
    return destination;
...
bool make_cached_png(const std::string &source, const PngDimensions &source_size, std::string &cached,
                     std::string &error)
{
    const auto source_pixels = static_cast<std::uint64_t>(source_size.width) * source_size.height;
    png_image decoded{};
    decoded.version = PNG_IMAGE_VERSION;
    if (!png_image_begin_read_from_memory(&decoded, source.data(), source.size())) {
        error = decoded.message;
        png_image_free(&decoded);
        return false;
    }
    if (decoded.width != source_size.width || decoded.height != source_size.height) {
    ...
    if (source_pixels > kCachedImageMaximumPixels) {
        const auto scale =
            std::sqrt(static_cast<double>(kCachedImageMaximumPixels) / static_cast<double>(source_pixels));
        destination_size = {std::max(1U, static_cast<std::uint32_t>(source_size.width * scale)),
                            std::max(1U, static_cast<std::uint32_t>(source_size.height * scale))};
        while (static_cast<std::uint64_t>(destination_size.width) * destination_size.height >
    ...
    if (!png_image_write_to_memory(&encoded, nullptr, &encoded_size, 0, output_pixels, 0, nullptr) ||
        encoded_size == 0) {
    ...
    if (!png_image_write_to_memory(&encoded, cached.data(), &encoded_size, 0, output_pixels, 0, nullptr)) {
        error = encoded.message;
        cached.clear();
        png_image_free(&encoded);
        return false;
...
const RegistryRelease *select_release(const RegistryApplication &app, const std::string &architecture,
                                      const std::string &appkit_version)
{
    const auto found = std::find_if(app.releases.cbegin(), app.releases.cend(), [&](const auto &release) {
        return release.status == "published" && release.architecture == architecture &&
               (appkit_version.empty() || debian_version_compare(release.min_appkit_version, appkit_version) <= 0);
    });
...
{
    if (const auto *value = std::getenv("LILYGO_UI_APPKIT_VERSION"); value && *value) return value;
...
StoreRefreshResult StoreService::refresh(const std::string &root_url, const std::string &root_document)
{
...
StoreRefreshResult StoreService::make_result(RegistryLoadResult loaded)
{
    ...
    if (architecture.empty()) return {{}, {}, "Unable to determine the device package architecture", {}, {}};
    ...
    for (const auto &registry_app : loaded.catalog.apps) {
        ...
        app.author         = registry_app.authors.empty() ? std::string{} : registry_app.authors.front().name;
        ...
        for (const auto &registry_screenshot : registry_app.screenshots) {
            ...
            screenshot.cache_key = loaded.catalog.snapshot_id + ":" + registry_screenshot.url;
            app.screenshots.push_back(std::move(screenshot));
        }
        const auto *release = select_release(registry_app, architecture, appkit_version);
        ...
        if (installed.installed) {
            ...
            app.update_available = release && debian_version_compare(installed.version, release->version) < 0;
        }
...
StoreRefreshResult refresh_store(const std::string &root_url)
{
    ...
    CurlHttpClient transport(runner);
    RecordingHttpClient http(transport);
    ...
    if (result) result.cache_error = RegistryRootCache{}.store(root_url, http.documents());
    return result;
}
...
StoreRefreshResult refresh_cached_store(const std::string &root_url)
{
    ...
    PosixProcessRunner runner;
    CachedRegistryHttpClient http(cache, root_url);
    RegistryClient registry(http);
    PackageManager packages(runner, http);
    return StoreService(registry, packages).refresh(root_url);
}

PackageOperationResult install_store_package(const std::string &package, const RegistryRelease &release)
{
    PosixProcessRunner runner;
...
StoreImageDownloadResult download_store_image(const std::string &url, const std::string &cache_key)
{
    if (cache_key.empty()) return {{}, "Image cache key is empty"};
    std::error_code error;
    const auto cache = image_cache_root(error);
    if (error || cache.empty()) return {{}, "Unable to locate the image cache directory"};
    const auto cache_status = std::filesystem::symlink_status(cache, error);
    if (!error && std::filesystem::is_symlink(cache_status)) return {{}, "The image cache directory is not safe"};
    error.clear();
    std::filesystem::create_directories(cache, error);
    if (error) return {{}, "Unable to create the image cache directory"};
    std::filesystem::permissions(cache, std::filesystem::perms::owner_all, std::filesystem::perm_options::replace,
                                 error);
    if (error) return {{}, "Unable to secure the image cache directory"};

    const auto destination = cache / (sha256(std::string(kImageCacheVersion) + cache_key) + ".png");
    if (destination.string().size() >= 240) return {{}, "The image cache path is too long"};
    const auto destination_status = std::filesystem::symlink_status(destination, error);
    if (!error && std::filesystem::is_regular_file(destination_status) && valid_cached_png(destination))
    ...
    if (!downloaded) return {{}, "Image download failed: " + downloaded.error};
    const auto *data = reinterpret_cast<const unsigned char *>(downloaded.body.data());
    PngDimensions source_size;
    if (!valid_png_data(data, downloaded.body.size(), &source_size)) return {{}, "Downloaded image is not a valid PNG"};
    std::string cached_image;
    std::string normalization_error;
    if (!make_cached_png(downloaded.body, source_size, cached_image, normalization_error))
    ...
    output.write(cached_image.data(), static_cast<std::streamsize>(cached_image.size()));
    output.close();
    if (!output) {
    ...
    if (error) {
        std::filesystem::remove(temporary, error);
        if (valid_cached_png(destination)) return {destination.string(), {}};
