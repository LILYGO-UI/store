...
void create_summary(lv_obj_t *body, const InstalledViewModel &view_model) {
  ...
  lv_obj_set_flex_align(summary, LV_FLEX_ALIGN_SPACE_BETWEEN,
                        LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);

  char count[40]{};
  const auto installed_count_value = view_model.installed_count();
  std::snprintf(count, sizeof(count), "%zu app%s installed",
                installed_count_value, installed_count_value == 1 ? "" : "s");
  ...
  std::snprintf(updates, sizeof(updates), "%zu update%s available",
                update_count_value, update_count_value == 1 ? "" : "s");
  auto *update_count_label =
      make_label(summary, updates, 14,
                 update_count_value ? COLOR_ACCENT : COLOR_SECONDARY);
...
void create_installed_row(lv_obj_t *body, std::size_t index,
                          const StoreApp &app,
  ...
  if (!app.operation_error.empty())
    ...
    std::snprintf(version, sizeof(version), "%s → %s · %s", app.version.c_str(),
                  app.latest_version.c_str(), app.size.c_str());
  else
    std::snprintf(version, sizeof(version), "Version %s · %s",
                  app.version.c_str(), app.size.c_str());
  auto *metadata = make_label(
      copy, version, 14,
      !app.operation_error.empty()
          ? COLOR_DANGER
          : (app.update_available ? COLOR_WARNING : COLOR_SECONDARY));
  ...
  lv_obj_set_size(actions, 116, 44);
  lv_obj_set_style_pad_column(actions, 8, 0);

  auto *primary = make_button(
      actions,
      app.busy ? "Working" : (app.update_available ? "Update" : "Open"), 14,
      COLOR_ACTION, COLOR_SURFACE, callbacks.primary_action,
      reinterpret_cast<void *>(static_cast<std::uintptr_t>(index)));
  lv_obj_set_size(primary, 64, 44);
  lv_obj_set_name_static(primary, "installed_primary_action");

  auto *remove =
      make_button(actions, LV_SYMBOL_TRASH, 22, COLOR_SURFACE, COLOR_DANGER,
                  callbacks.remove,
                  reinterpret_cast<void *>(static_cast<std::uintptr_t>(index)));
...
void create_storage(lv_obj_t *body) {
  ...
  auto *status =
      make_button(card, "Plenty", 14, COLOR_SUCCESS_SOFT, COLOR_SUCCESS);
...
void build_installed_view(lv_obj_t *content, InstalledViewModel &view_model,
                          const InstalledViewCallbacks &callbacks) {
  ...
  for (std::size_t index = 0; index < view_model.apps().size(); ++index) {
    ...
    if (app.installed)
      create_installed_row(body, index, app, callbacks);
  }
...
InstalledView::InstalledView(InstalledViewModel &view_model,
...
void InstalledView::revision_changed(lv_observer_t *observer, lv_subject_t *) {
  auto *view =
      static_cast<InstalledView *>(lv_observer_get_user_data(observer));
