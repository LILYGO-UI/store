...
void create_identity(lv_obj_t *body, const StoreApp &app, std::size_t index) {
  ...
  lv_obj_set_flex_align(identity_row, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER,
  ...
  const char *publisher =
      app.author.empty() ? "Unknown publisher" : app.author.c_str();
  if (app.official)
    publisher = "LILYGO Official";
  auto *publisher_label = make_label(
      copy, publisher, 14, app.official ? COLOR_ACCENT : COLOR_SECONDARY);
...
void populate_screenshot_card(lv_obj_t *card,
                              const StoreScreenshot &screenshot) {
  lv_obj_clean(card);
  ...
  lv_obj_set_flex_align(fallback, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER,
                        LV_FLEX_ALIGN_CENTER);
  make_label(fallback, LV_SYMBOL_FILE, 28,
             screenshot.load_error.empty() ? COLOR_ACCENT : COLOR_SECONDARY);
  auto *caption =
      make_label(fallback, screenshot.caption.c_str(), 14, COLOR_TEXT);
  ...
  make_label(fallback,
             screenshot.load_error.empty() ? "Loading..." : "Unavailable", 14,
             COLOR_SECONDARY);
}
...
void create_facts(lv_obj_t *body, const StoreApp &app) {
  ...
  lv_obj_set_flex_align(facts, LV_FLEX_ALIGN_SPACE_BETWEEN,
                        LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
  char version[40]{};
...
void create_actions(lv_obj_t *body, const StoreApp &app,
                    const DetailViewCallbacks &callbacks) {
  ...
  auto *primary = make_button(body, primary_text, 22, COLOR_ACTION,
                              COLOR_SURFACE, callbacks.primary_action);
  lv_obj_set_name_static(primary, "primary_action");
  lv_obj_set_size(primary, LV_PCT(100), 56);
  if (app.busy || !app.available || (app.installed && !app.update_available))
    lv_obj_clear_flag(primary, LV_OBJ_FLAG_CLICKABLE);

  if (!app.installed)
    return;
  auto *remove = make_button(body, LV_SYMBOL_TRASH "  Delete App", 22,
                             COLOR_SURFACE, COLOR_DANGER, callbacks.remove);
...
void create_operation_error(lv_obj_t *body, const StoreApp &app) {
  ...
  auto *message =
      make_label(notice, app.operation_error.c_str(), 14, COLOR_DANGER);
...
void create_source_notice(lv_obj_t *body, const StoreApp &app) {
  ...
  lv_obj_set_flex_align(notice, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER,
                        LV_FLEX_ALIGN_CENTER);
  ...
  const char *message =
      app.official
          ? "This app is published by LILYGO."
...
const char *permission_name(const std::string &id) {
  ...
  if (id == "imu")
    return "IMU";
  if (id == "external-display")
    return "External Display";
  if (id == "background-service")
...
void create_permissions(lv_obj_t *body, const StoreApp &app) {
  ...
  const auto card_height = static_cast<int>(
      card_vertical_padding + rows * row_height + (rows - 1) * row_gap);
  ...
  for (const auto &permission : app.permissions) {
    ...
    auto *reason =
        make_label(row, permission.reason.c_str(), 14, COLOR_SECONDARY);
...
void build_detail_view(lv_obj_t *content, DetailViewModel &view_model,
                       const DetailViewCallbacks &callbacks) {
...
DetailView::DetailView(DetailViewModel &view_model,
...
void DetailView::rebuild() {
  ...
  if (auto *body = lv_obj_find_by_name(surface_, "page_body"))
    body_scroll_y = lv_obj_get_scroll_y(body);
  ...
  if (auto *body = lv_obj_find_by_name(surface_, "page_body"))
    lv_obj_scroll_to_y(body, body_scroll_y, LV_ANIM_OFF);
  if (auto *gallery = lv_obj_find_by_name(surface_, "screenshot_gallery"))
