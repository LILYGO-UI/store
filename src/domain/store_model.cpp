...
constexpr const char *kLauncherPackage = "lilygo-ui-launcher";

}  // namespace

bool is_launcher_package(const std::string &package) noexcept
{
    return package == kLauncherPackage;
}

bool is_removable_package(const std::string &package) noexcept
{
    return package != LILYGO_UI_STORE_PACKAGE_NAME && !is_launcher_package(package);
}

StoreModel::StoreModel(std::vector<StoreApp> apps)
    : apps_(std::move(apps)), registry_state_(RegistryState::ready), status_message_()
{
}

CatalogFilter StoreModel::filter() const noexcept
{
    return filter_;
}

const std::vector<StoreApp> &StoreModel::apps() const noexcept
{
    return apps_;
}
...
{
    const auto found =
        std::find_if(apps_.cbegin(), apps_.cend(), [&](const auto &item) { return item.app_id == selected_app_id_; });
...
std::size_t StoreModel::selected_index() const noexcept
{
    const auto found =
        std::find_if(apps_.cbegin(), apps_.cend(), [&](const auto &item) { return item.app_id == selected_app_id_; });
    return found == apps_.cend() ? apps_.size() : static_cast<std::size_t>(std::distance(apps_.cbegin(), found));
}

std::size_t StoreModel::installed_count() const noexcept
{
    return static_cast<std::size_t>(
        std::count_if(apps_.cbegin(), apps_.cend(), [](const auto &item) { return item.installed; }));
}

std::size_t StoreModel::update_count() const noexcept
{
    return static_cast<std::size_t>(std::count_if(
        apps_.cbegin(), apps_.cend(), [](const auto &item) { return item.installed && item.update_available; }));
}

bool StoreModel::matches_filter(const StoreApp &item) const noexcept
{
    if (filter_ == CatalogFilter::official) return item.official;
    if (filter_ == CatalogFilter::community) return !item.official;
    return true;
}
...
void StoreModel::set_filter(CatalogFilter filter) noexcept
{
    filter_ = filter;
}
...
void StoreModel::replace_catalog(std::vector<StoreApp> apps, std::string message)
{
    apps_           = std::move(apps);
    registry_state_ = RegistryState::ready;
    status_message_ = std::move(message);
    if (!selected_app_id_.empty() && !selected_app()) selected_app_id_.clear();
}
...
{
    auto *item = app(index);
    if (!item || !item->installed || !item->update_available) return false;
    item->version = version.empty() ? item->latest_version : std::move(version);
    item->operation_error.clear();
    item->update_available = false;
    item->busy             = false;
