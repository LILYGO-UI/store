...
std::string registry_root_url() {
  if (const auto *configured = std::getenv("LILYGO_UI_STORE_REGISTRY_URL");
      configured && *configured)
    return configured;
  return kDefaultRegistryRoot;
}

void deliver_async(const std::shared_ptr<AsyncMailbox> &mailbox,
                   AsyncResult result) {
...
void apply_async_result(AsyncResult result) {
  if (!app_state.store || app_state.closing)
    return;
  if (result.kind == AsyncResultKind::refresh) {
    if (result.cached_refresh) {
      if (app_state.online_refresh_succeeded || !result.refresh)
        return;
    ...
    if (result.refresh) {
      if (!result.refresh.cache_error.empty())
        LV_LOG_WARN("Unable to cache Registry root: %s",
                    result.refresh.cache_error.c_str());
      app_state.store->replace_catalog(std::move(result.refresh.apps));
      start_app_icon_downloads();
    } else {
    ...
    if (app_state.router.page() == StorePage::detail)
      start_detail_screenshot_downloads();
    return;
  }

  if (result.kind == AsyncResultKind::icon) {
    app_state.store->complete_icon(
        result.app_id, result.image_url, result.image_cache_key,
        std::move(result.image.path), std::move(result.image.error));
    return;
  }

  if (result.kind == AsyncResultKind::screenshot) {
    app_state.store->complete_screenshot(
        result.app_id, result.image_index, result.image_url,
  ...
  if (!result.package) {
    app_state.store->fail_operation(result.app_id,
                                    std::move(result.package.error));
    return;
  }
  if (result.kind == AsyncResultKind::remove)
    app_state.store->complete_remove(result.app_id);
  else
    app_state.store->complete_install(
        result.app_id, result.was_update,
...
void start_registry_refresh(bool use_cache) {
  if (!app_state.store || app_state.refresh_running ||
      app_state.operation_running)
    return;
...
void start_detail_screenshot_downloads() {
  if (!app_state.store || !app_state.detail)
    return;
  auto pending = app_state.store->begin_screenshot_downloads(
      app_state.detail->app_index());
  if (pending.empty())
    return;
...
void start_install(std::size_t index) {
  if (!app_state.store)
    return;
  auto command = app_state.store->begin_install(
      index, app_state.operation_running, app_state.refresh_running);
  if (!command)
    return;
...
void start_remove(std::size_t index) {
  if (!app_state.store)
    return;
  auto command = app_state.store->begin_remove(
      index, app_state.operation_running, app_state.refresh_running);
  if (!command)
    return;
  app_state.operation_running = true;
...
void select_app_event(lv_event_t *event) {
  if (!app_state.catalog || !app_state.catalog->select(event_index(event)))
    return;
...
void filter_event(lv_event_t *event) {
  const auto filter = static_cast<CatalogFilter>(
      reinterpret_cast<std::intptr_t>(lv_event_get_user_data(event)));
  if (app_state.catalog)
    app_state.catalog->set_filter(filter);
}
...
void navigation_event(lv_event_t *event) {
  const auto page = static_cast<StorePage>(
      reinterpret_cast<std::intptr_t>(lv_event_get_user_data(event)));
...
void back_event(lv_event_t *) {
  if (app_state.router.back())
    request_page_render();
}

void detail_primary_action_event(lv_event_t *) {
  if (app_state.detail)
    start_install(app_state.detail->app_index());
}

void detail_remove_event(lv_event_t *) {
  if (app_state.detail)
    start_remove(app_state.detail->app_index());
}

void refresh_event(lv_event_t *) { start_registry_refresh(); }

StorePage active_navigation_page() {
  return app_state.router.page() == StorePage::detail
             ? app_state.router.detail_origin()
             : app_state.router.page();
}
...
void create_navigation() {
  ...
  for (const auto &item : items) {
    ...
    lv_obj_set_name_static(button, item.page == StorePage::catalog
                                       ? "navigation_store"
    ...
    lv_obj_add_event_cb(
        button, navigation_event, LV_EVENT_CLICKED,
        reinterpret_cast<void *>(static_cast<std::intptr_t>(item.page)));
    auto *icon = make_label(button, item.icon, 22,
                            selected ? COLOR_ACCENT : COLOR_SECONDARY);
    lv_obj_set_name_static(icon, "navigation_icon");
    auto *label = make_label(button, item.label, 14,
                             selected ? COLOR_ACCENT : COLOR_SECONDARY);
...
void render_page_now() {
  if (app_state.closing || !app_state.content || !app_state.navigation)
    return;

  if (app_state.catalog_view)
    app_state.catalog_view->destroy();
  if (app_state.installed_view)
    app_state.installed_view->destroy();
  if (app_state.detail_view)
    app_state.detail_view->destroy();
...
void request_page_render() {
  if (app_state.closing || app_state.page_render_pending || !app_state.content)
    return;
  app_state.page_render_pending =
      lv_async_call(render_page_async, &app_state) == LV_RESULT_OK;
}
...
void apply_layout() {
  if (!app_state.surface || !app_state.content || !app_state.navigation)
    return;

  const auto width = lv_obj_get_content_width(app_state.surface);
  const bool wide = width >= 720;
  lv_obj_set_flex_flow(app_state.surface,
                       wide ? LV_FLEX_FLOW_ROW : LV_FLEX_FLOW_COLUMN);

  lv_obj_set_flex_align(app_state.content, LV_FLEX_ALIGN_START,
  ...
  if (wide) {
    ...
    for (std::uint32_t index = 0;
         index < lv_obj_get_child_count(app_state.navigation); ++index) {
...
void size_changed_event(lv_event_t *) { apply_layout(); }

void app_close();

void app_open(const cm0_app_context_t *context) {
  if (app_state.surface || app_state.store)
    app_close();
  ...
  app_state.catalog_view = std::make_unique<CatalogView>(
      *app_state.catalog,
      CatalogViewCallbacks{select_app_event, catalog_quick_action_event,
                           filter_event, refresh_event, apply_layout});
  app_state.installed_view = std::make_unique<InstalledView>(
      *app_state.installed,
      InstalledViewCallbacks{select_app_event, installed_primary_action_event,
                             installed_remove_event, apply_layout});
  app_state.detail_view = std::make_unique<DetailView>(
      *app_state.detail,
      DetailViewCallbacks{back_event, detail_primary_action_event,
                          detail_remove_event, apply_layout});
  ...
  lv_obj_set_style_border_color(app_state.navigation,
                                lv_color_hex(COLOR_SEPARATOR), 0);
...
void app_close() {
  app_state.closing = true;
  if (app_state.mailbox)
    app_state.mailbox->accepting = false;
  lv_async_call_cancel(render_page_async, &app_state);
  app_state.page_render_pending = false;
  if (app_state.worker_timer)
    lv_timer_delete(app_state.worker_timer);
...
const cm0_app_descriptor_t app_descriptor{
    CM0_APP_API_VERSION, sizeof(cm0_app_descriptor_t), LILYGO_UI_STORE_APP_ID,
    app_open, app_close};
