...
void Sha256::update(const void *data, std::size_t size) noexcept {
  if (finished_ || !data || size == 0)
    return;
  const auto *bytes = static_cast<const std::uint8_t *>(data);
  bit_count_ += static_cast<std::uint64_t>(size) * 8U;
  while (size > 0) {
    const auto available = sizeof(buffer_) - buffer_size_;
    const auto amount = size < available ? size : available;
...
std::string Sha256::finish() noexcept {
  if (!finished_) {
    ...
    if (buffer_size_ > 56) {
      while (buffer_size_ < sizeof(buffer_))
    ...
    for (int shift = 56; shift >= 0; shift -= 8)
      buffer_[buffer_size_++] =
          static_cast<std::uint8_t>((message_bits >> shift) & 0xffU);
    transform(buffer_);
    finished_ = true;
  }
...
std::string sha256(std::string_view value) noexcept {
  Sha256 digest;
...
bool sha256_file(const std::filesystem::path &path, std::string &digest,
  ...
  if (!input) {
    error = "Unable to read the downloaded file";
    return false;
  }
  Sha256 value;
  ...
  while (input) {
    ...
    value.update(buffer.data(), static_cast<std::size_t>(input.gcount()));
  }
  if (!input.eof()) {
    error = "Failed while reading the downloaded file";
    return false;
  }
  digest = value.finish();
  return true;
