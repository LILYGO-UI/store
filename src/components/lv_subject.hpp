#ifndef LILYGO_UI_STORE_COMPONENTS_LV_SUBJECT_HPP
#define LILYGO_UI_STORE_COMPONENTS_LV_SUBJECT_HPP
...
class IntSubject {
public:
  explicit IntSubject(std::int32_t value, std::int32_t minimum = INT32_MIN,
                      std::int32_t maximum = INT32_MAX) {
  ...
  ~IntSubject() { lv_subject_deinit(&subject_); }
  ...
  void set(std::int32_t value) { lv_subject_set_int(&subject_, value); }

  [[nodiscard]] std::int32_t value() const {
    return lv_subject_get_int(const_cast<lv_subject_t *>(&subject_));
  }

  [[nodiscard]] lv_subject_t *get() noexcept { return &subject_; }
